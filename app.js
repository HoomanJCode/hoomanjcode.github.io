...
if (pongCanvas) {
  ...
  const resetBall = (direction) => {
    ball = { x: .5, y: .5, vx: direction * (.3 + Math.random() * .08), vy: (Math.random() - .5) * .34 };
    trail.length = 0;
  };
  ...
  const drawPong = (now) => {
    ...
    lastTime = now;
    const paddleW = Math.max(5, width * .018), paddleH = Math.max(44, height * .24);
    const px = width * .09, ax = width * .91;
    player += (playerTarget - player) * Math.min(1, dt * 12);
    ai += (ball.y - ai) * Math.min(1, dt * (reducedMotion ? 3 : 4.2));
    ai = Math.max(.12, Math.min(.88, ai));
    if (!reducedMotion) { ball.x += ball.vx * dt; ball.y += ball.vy * dt; }
    if (ball.y < .08 || ball.y > .92) { ball.vy *= -1; ball.y = Math.max(.08, Math.min(.92, ball.y)); }
    const playerHit = ball.x < .12 && ball.x > .095 && Math.abs(ball.y - player) < .16;
    const aiHit = ball.x > .88 && ball.x < .905 && Math.abs(ball.y - ai) < .16;
    if (playerHit && ball.vx < 0) { ball.vx = Math.abs(ball.vx) * 1.04; ball.vy += (ball.y - player) * .28; score++; }
    if (aiHit && ball.vx > 0) { ball.vx = -Math.abs(ball.vx) * 1.04; ball.vy += (ball.y - ai) * .22; aiScore++; }
    if (ball.x < -.04) resetBall(1);
    if (ball.x > 1.04) resetBall(-1);
    ...
    ctx.beginPath(); ctx.moveTo(width / 2, height * .12); ctx.lineTo(width / 2, height * .88); ctx.stroke(); ctx.setLineDash([]);
    ...
    ctx.fillStyle = '#d5ff4f'; ctx.shadowColor = '#d5ff4f'; ctx.shadowBlur = 16; ctx.fillRect(px - paddleW / 2, player * height - paddleH / 2, paddleW, paddleH); ctx.shadowBlur = 0;
    ctx.fillStyle = '#ff765c'; ctx.shadowColor = '#ff765c'; ctx.shadowBlur = 16; ctx.fillRect(ax - paddleW / 2, ai * height - paddleH / 2, paddleW, paddleH); ctx.shadowBlur = 0;
